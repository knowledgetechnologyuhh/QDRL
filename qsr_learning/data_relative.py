...
from qsr_learning.relation_relative import left_of, right_of, in_front_of, behind


Question = namedtuple("Question", ["entity1", "relation", "entity2", "entity3"])
...
class DRLDataset(Dataset):
    def __init__(
        ...
        relation_names=["in_front_of", "behind", "left_of", "right_of"],
        ...
        self.excluded_triples = set(product(excluded_entity_names, repeat=3))
        self.relation_names = set(relation_names)
        self.relations = [
            getattr(qsr_learning.relation_relative, relation_name)
    ...
    def gen_sample(self, idx):
        ...
        quadruple_found = False

        while not quadruple_found:
            entity_names = random.sample(self.entity_names, self.num_entities)
            entity1_name, entity2_name, entity3_name = random.sample(entity_names, 3)
            if (entity1_name, entity2_name, entity3_name) in self.excluded_triples:
                if self.allowed_relation_names:
                    relation = random.choice(self.allowed_relations)
                    quadruple_found = True
            else:
        ...
        while not sample_found:
            ...
            for entity in entities:
                if entity.name == entity1_name:
                    entity1 = entity
                elif entity.name == entity2_name:
                    entity2 = entity
                elif entity.name == entity3_name:
                    entity3 = entity
                else:
                    pass
            if any(
                r(entity1, entity2, entity3)
                for r in [left_of, right_of, in_front_of, behind]
            ):
                sample_found = relation(entity1, entity2, entity3) == answer
        image = draw_entities(
            entities,
            canvas_size=self.canvas_size,
        ...
        question = Question(entity1.name, relation.__name__, entity2.name, entity3.name)
